...
function generateRandomSwwwOptions() {
    ...
    const outputs = '0,1'; // Comma-separated list of outputs (e.g., "0,1")
    ...
    const transitionTypes = ['none', 'simple', 'fade', 'left', 'right', 'top', 'bottom', 'wipe', 'wave', 'grow', 'center', 'any', 'outer', 'random']; // Transition types
    ...
    const options = {
        ...
        transitionPos: getRandomElement(['center', 'top', 'left', 'right', 'bottom', 'top-left', 'top-right', 'bottom-left', 'bottom-right', `${Math.random() * 100},${Math.random() * 100}`]) // Center position for transitions
    };
...
export async function setWallpaper(wallpaperPath) {
    const command = createSwwwCommand(wallpaperPath, generateRandomSwwwOptions(wallpaperPath));
